...
void Promise.all([getFontSize(), windowSizeTools.init()]).then(async ([fontSize]) => {
  ...
  const { init: initNavigation, navigations, pushHomeTVScreen } = await import('@/navigation')
  ...
  initNavigation(async () => {
    ...
    const launchPromise = Platform.isTV ? pushHomeTVScreen() : navigations.pushHomeScreen()
